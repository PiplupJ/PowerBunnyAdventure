    ...
    IDropItemSystem
{
    ...
    {
        EnemyData enemyData_ = enemyDataManager.GetEnemyDataByID(enemyID);
        if(enemyData_ == null)
        {
            Debug.LogError($"[EntityManager]EnemyData{enemyID}に相当する敵がありません");
            return;
        }

        //ObjectPoolから敵生成
        if(!ObjectPool.Instance.TryGetObject<Enemy>(enemyID, out Enemy enemy))
        {
            Debug.LogError($"[EntityManager]{enemyID}の敵がスポーン出来ませんでした");
            return;
        }
    ...
    public void CreatePlayerShot(int id, Vector3 spawnPos, int playerAttack, Vector3 shotDir)
    {
        ...
        if(!ObjectPool.Instance.TryGetObject<Shot>(id, out Shot pShot))
        {
            Debug.LogError($"[EntityManager]PlayerShot{id}に相当する弾がスポーン出来ませんでした");
            return;
        }
        pShot.transform.position = spawnPos;
        bool isPlayerShot = true;
        pShot.SetShotData(shotData, _mapSystem, this, isPlayerShot);
    ...
    public void CreateEnemyShot(int id, Vector3 spawnPos, int enemyAttack, Vector3 shotDir)
    {
        ...
        {
            Debug.LogError($"[EntityManager]EnemyShot{id}に相当する弾がスポーン出来ませんでした");
            return;
        }
        eShot.transform.position = spawnPos;
        bool isPlayerShot = false;
        eShot.SetShotData(shotData, _mapSystem, this, isPlayerShot);
    ...
    public Shot CreateEnemyHitBox(int id, Vector3 spawnPos, int enemyAttack)
    {
        ...
        {
            Debug.LogError($"[EntityManager]EnemyShot{id}に相当する弾がありませんでした");
            return null;
        }
        eHitBox.transform.position = spawnPos;
        bool isPlayerShot = false;
        eHitBox.SetShotData(shotData, _mapSystem, this, isPlayerShot);
    ...
    {
        if(!ObjectPool.Instance.TryGetObject<DropItem>(itemID, out DropItem item))
        {
            Debug.LogError($"[EntityManager]DropItem {itemID}のアイテムがスポーン出来ませんでした");
            return null;
        }
