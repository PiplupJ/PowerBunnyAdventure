...
public class LevelManager : MonoBehaviour , ILevelUpSystem
{
    ...
    //レベルアップの時、ObjectPoolから求める演出用オブジェクトのID
    const int effectID = 50000; //임시
    ...
    private void LoadExpTable()
    {
        ...
        if (jsonText != null)
        {
            ...
            Debug.Log($"jsonファイルロード完了。最大レベル： {maxLevel}");
        }
        else
        {
            Debug.LogError("ExpTable.jsonファイルはありません");
        }
    }
    ...
    {
        HitEffect levelUp = ObjectPool.Instance.GetObject<HitEffect>(IDRegistry.EFFECT_LEVELUP);
