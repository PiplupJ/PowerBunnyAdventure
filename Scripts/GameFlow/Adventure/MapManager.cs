...
public class MapManager : IMapSystem
{
    ...
    private void LoadRoomDatas()
    {
        ...
        if (jsonText == null)
        {
            throw new System.IO.FileNotFoundException("部屋データが見つかりません:Resources/Data/RoomDatas");
        }

        RoomDataList list = JsonUtility.FromJson<RoomDataList>(jsonText.text);
        if(list?.rooms == null||list.rooms.Count == 0)
        {
            throw new System.Exception("部屋データのパースに失敗、または空きです:Data/RoomDatas");
        }
    ...
    public void MapInit(int RoomID, StageType type)
    {
            ...
            for(int x = 0; x < mapWidth; x++)
            {
                ...
                switch(tileData)
                {
                    case TileType.START :
                        ...
                        if(ObjectPool.Instance.TryGetObject<PoolableObject>(portalIDs[type], out startPortal))
                        startPortal.transform.position = startPos;
                        break;
                    case TileType.GOAL :
                        goalPos = GridToWorldSpace(x, z);
                        if(ObjectPool.Instance.TryGetObject<PoolableObject>(IDRegistry.GATE_GOAL, out goalGate))
        ...
        ObjectPool.Instance.TryGetObject<PoolableObject>(RoomID, out currentRoom);
    }
    //現在のマップを削除
    ...
    {
        if(ObjectPool.Instance.TryGetObject<PoolableObject>(IDRegistry.GATE_START, out startGate))
        {
    ...
    public void OpenGate(StageType nextStage)
    {
        ...
        if(ObjectPool.Instance.TryGetObject<PoolableObject>(portalIDs[nextStage], out goalPortal))
        {
